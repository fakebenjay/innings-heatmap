...
def tally_runs(pbp, status):
    ...
    key = "away_score" if status == "top" else "home_score"
...
for g in done_regular_games_2026:
    ...
    away_line_raw = list(map(lambda x: safe_integer(x), linescore_list[1].split(" ")))
    home_line_raw = list(map(lambda x: safe_integer(x), linescore_list[2].split(" ")))
    ...
    if verify(away_innings) or verify(home_innings):
        ...
        away_innings = tally_runs(pbp, "top")[: g["current_inning"]]
        home_innings = (
    ...
    away_extras_runs = sum(away_extras)
    home_extras_runs = sum(home_extras)
